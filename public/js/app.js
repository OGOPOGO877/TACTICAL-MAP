...
async function main() {
  ...
  const serverBuild = appConfig.buildId;
  const pageBuild = window.__TACTICAL__?.buildId;
  if (serverBuild && serverBuild !== pageBuild) {
    const key = "tactical-build-reload";
    if (sessionStorage.getItem(key) !== serverBuild) {
